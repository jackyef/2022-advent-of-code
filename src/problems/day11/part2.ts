  ...
  return {
    ...
    makeHumanWorry: (() => {
      ...
      return (prevWorryLevel: number) => {
        return eval(`var old = ${prevWorryLevel}; Number(${formulaString});`)
      }
    })(),
    decideMonkeyToThrowTo: (() => {
      const testDivisibilityNumber = lines[3].trim().replace("Test: divisible by ", "")
      dividers.push(Number(testDivisibilityNumber))
...
let currentRound = 1;
let maxNumberToCheck = dividers.reduce((acc, v) => acc * v, 1)
...
while (currentRound <= 10000) {
  for (const [index, monkey] of Object.entries(monkeys)) {
    ...
    while (item = monkey.items.shift()) {
      ...
      // monkey gets bored
      // We don't need to check further than the maxNumber
      // so we can just check for the modulo of it.
      worryLevel = worryLevel % maxNumberToCheck
